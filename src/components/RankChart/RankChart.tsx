...
function TrophyIconSVG({ x, y, className }: { x: number; y: number; className: string }) {
  // Reuse the same trophy silhouette as the counters (24x24 path) and scale it down
  // so it fits nicely inside the 10px gold circle.
  const scale = 0.42;
  const size = 24 * scale;
  const tx = x - size / 2;
  const ty = y - size / 2;
  ...
  return (
    <g className={className} transform={`translate(${tx}, ${ty}) scale(${scale})`}>
      <path
